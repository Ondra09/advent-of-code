(ns advent.day3
  (:require [advent.simple2D :as s2D]))
(use 'clojure.test)
...
(defn decode-direction [dir]
  (let [dir-name (name dir)
        ...
        rest (read-string (subs dir-name 1))]
  (cond
    (= first-char "L") {:x (- rest) :y 0}
    (= first-char "R") {:x rest :y 0}
    (= first-char "D") {:x 0 :y (- rest)}
    (= first-char "U") {:x 0 :y rest}
    :else nil)))


(deftest decode-direction-test
  (is (= (decode-direction "L34") {:x -34 :y 0}))
  (is (= (decode-direction "R4") {:x 4 :y 0}))
  (is (= (decode-direction "U343") {:x 0 :y 343}))
  (is (= (decode-direction "D43") {:x 0 :y -43}))
  (is (= (decode-direction "K43") nil)))

(defn add-to-last [buffer direction]
  (when-let [last-point (last buffer)]
    { :x (+ (:x last-point) (:x direction))
     :y (+ (:y last-point) (:y direction))}))

;; (add-to-last [{:x 1 :y 10}] {:x 50 :y 10})

(defn convert-to-list-of-points [list-of-diffs]
  (reduce (fn [buffer x]
            (if-let [new-item (add-to-last buffer x)]
              (conj buffer new-item)))
          [{:x 0 :y 0}] ;; first point
          list-of-diffs))

(convert-to-list-of-points [{:x 0 :y -2} {:x 0 :y 10} {:x -50 :y 0}])
...
(defn find-all-intersections [a0 a1 list]
  (let [line-intersection (partial s2D/line-line-intersection a0 a1)]
    (reduce
     (fn [buffer point]
       (conj buffer (line-intersection (first point) (second point))))
     []
     (partition 2 1 list))
    ))

(defn compute-all-intersections [list1 list2]
  (reduce
   (fn [buffer point]
     (concat buffer (find-all-intersections (first point) (second point) list1)))
   []
   (partition 2 1 list2)))


(defn find-minimal-manhattan-distance [list]
  "distance 0 is ignored"
  (reduce
   (fn [min point]
     (let [new-min  (+ (s2D/abs (:x point)) (s2D/abs (:y point)))]
       (if (and (not= 0 new-min)
                (<  new-min min))
         new-min
         min)))
   9999999
...
;;
(defn compute-nearest-intersection [list1 list2]
  (let [converted-list-1 (convert-to-list-of-points (map decode-direction list1))
        converted-list-2 (convert-to-list-of-points (map decode-direction list2))]
    (find-minimal-manhattan-distance (filter some? (compute-all-intersections converted-list-1 converted-list-2)))))

(deftest compute-nearest-intersection-test
  (is (= (:distance test-input-2)
         (compute-nearest-intersection (:L1 test-input-2) (:L2 test-input-2))))
  (is (= (:distance test-input-1)
         (compute-nearest-intersection (:L1 test-input-1) (:L2 test-input-1)))))

(compute-nearest-intersection L1 L2)

(defn point-distance-on-path [p list]
  (reduce
     (fn [sum point]
       (let [on-line  (s2D/point-inside-line p (first point) (second point))]
         (if on-line
           (reduced (+ sum (s2D/manhattan-distance (first point) p)))
           (+ sum (s2D/manhattan-distance (first point) (second point))))))
     0
     (partition 2 1 list)))

(defn find-shortest-path [list1 list2]
  (let [converted-list-1 (convert-to-list-of-points (map decode-direction list1))
        converted-list-2 (convert-to-list-of-points (map decode-direction list2))
        intersections (filter some? (compute-all-intersections converted-list-1 converted-list-2))]

    (map
     (fn [intersection] (+
             (point-distance-on-path intersection converted-list-1)
             (point-distance-on-path intersection converted-list-2)))
     intersections)))

(apply min (find-shortest-path L1 L2))
